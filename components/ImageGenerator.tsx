
import React, { useState } from 'react';
import { generateImage } from '../services/geminiService';
import type { AspectRatio } from '../types';
import { IMAGEN_ASPECT_RATIOS } from '../constants';
import { PromptInput } from './common/PromptInput';

export const ImageGenerator: React.FC = () => {
  const [prompt, setPrompt] = useState('');
  const [aspectRatio, setAspectRatio] = useState<AspectRatio>('1:1');
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [generatedImages, setGeneratedImages] = useState<string[]>([]);

  const handleSubmit = async () => {
    if (!prompt.trim()) {
      setError('Please enter a prompt.');
      return;
    }
    setIsLoading(true);
    setError(null);
    setGeneratedImages([]);
    try {
      const images = await generateImage(prompt, aspectRatio, 1);
      setGeneratedImages(images);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An unknown error occurred.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div className="max-w-4xl mx-auto flex flex-col h-full">
      <div className="flex-grow overflow-y-auto pr-2">
        <h2 className="text-2xl font-bold text-gray-100 mb-4">Image Generator</h2>
        <p className="text-gray-400 mb-6">Create stunning visuals from text. Describe anything you can imagine, and watch it come to life.</p>
        
        {error && <div className="bg-red-900 border border-red-700 text-red-200 px-4 py-3 rounded-lg relative mb-4" role="alert">{error}</div>}

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center mb-6">
            <div className="text-gray-300">
                <label htmlFor="aspect-ratio" className="block text-sm font-medium mb-2">Aspect Ratio</label>
                 <select
                    id="aspect-ratio"
                    value={aspectRatio}
                    onChange={(e) => setAspectRatio(e.target.value as AspectRatio)}
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                >
                    {IMAGEN_ASPECT_RATIOS.map(ratio => (
                        <option key={ratio.value} value={ratio.value}>{ratio.name}</option>
                    ))}
                </select>
            </div>
        </div>

        {isLoading && (
            <div className="flex justify-center items-center h-64 bg-gray-800 rounded-lg">
                <div className="text-center">
                    <svg className="animate-spin mx-auto h-10 w-10 text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    <p className="mt-2 text-gray-300">Generating...</p>
                </div>
            </div>
        )}

        {generatedImages.length > 0 && (
          <div className="grid grid-cols-1 gap-4 mt-6">
            {generatedImages.map((image, index) => (
              <div key={index} className="bg-gray-800 rounded-lg p-2">
                <img src={image} alt={`Generated image ${index + 1}`} className="rounded-md w-full object-contain" />
              </div>
            ))}
          </div>
        )}
      </div>

      <div className="flex-shrink-0 pt-4 border-t border-gray-700">
        <PromptInput 
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          onSubmit={handleSubmit}
          isLoading={isLoading}
          placeholder="e.g., A cinematic shot of a raccoon in a tiny detective trench coat..."
        />
      </div>
    </div>
  );
};
